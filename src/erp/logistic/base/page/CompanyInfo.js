...
function CompanyInfo(props) {
    ...
    //mui TablePagination의 onPageChange 두 번째 파라미터는 page number 전달
    ...
    useEffect(() => {
        axios
            .get(
                'http://localhost:9102/compinfo/company/list'
                            ...
                            {dataList.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((dataList) => (
                                <TableRow sx={{ py: 3 }} hover role="checkbox" tabIndex={-1} key={dataList.code}>
                                    {columns.map((column) => {
                                        ...
                                        return (
                                            <TableCell key={column.id} align={column.align} onClick={(e) => onCellClicked(column)}>
